...
Place geometric shapes into a 3D voxel grid. Each shape carries a fill value or a gradient, and boundary voxels can be blended when a surface doesn't line up with the grid. `rasterize` runs the fill and gives you a plain Julia array.
...
A [`Geometry`](@ref) says what to draw: a tuple of shapes, a background value, and an anti-aliasing strategy. It says nothing about where the samples are taken, so the same geometry can be rasterized at any resolution. It's immutable, so use [`add_shape`](@ref) to build one up incrementally:
...
A [`Region`](@ref) says where to sample: a cell count, a voxel side length, and a center, all in exact rational arithmetic. The region spans `center - cells .* scale / 2` to `center + cells .* scale / 2`, so a region centered on `(1/2, 1/2, 1/2)` with `N` unit-fraction voxels per axis covers the unit cube:

```julia
region = Region((N, N, N), (1//N, 1//N, 1//N), (1//2, 1//2, 1//2))
```

[`rasterize`](@ref) fills every voxel center of a region with the geometry's value there. See [Grids and refinement](@ref) for regions of mixed resolution.
