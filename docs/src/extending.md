...
The default returns the all-infinite box `((-Inf,-Inf,-Inf), (Inf,Inf,Inf))`, so [`refine`](@ref) around a shape without this method just refines the whole grid. A tighter, conservative box (never smaller than the shape, larger is fine) lets `refine` carve out only the region around the shape instead. See [Grids and refinement](@ref) for how the box is used.
...
For the shape to work with `rasterize(geometry, region, CuArray)`, the struct must be `isbits`. Avoid fields that are heap-allocated (arrays, strings, arbitrary closures). Use `StaticArrays.SVector` for fixed-size vector fields and `NTuple` for fixed-size tuples.
